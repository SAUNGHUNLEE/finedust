...
public interface FineDustRepository extends JpaRepository<FineDust, Integer> {
    ...
    //경보발령
    @Query(value = "SELECT * FROM fine_dust WHERE measurement_name = :measurementName AND ((pm10 >= 150 OR pm10 >= 300) OR (pm2_5 >= 75 OR pm2_5 >= 150)) AND date >= :beforeDate AND date <= :currentDate", nativeQuery = true)
    ...
    //pm10,pm2.5가 모두 0인경우
    @Query(value = "SELECT * FROM fine_dust WHERE measurement_name = :measurementName AND pm10 = 0 AND pm2_5 = 0",nativeQuery = true)
    List<FineDust> findCheckDay(@Param("measurementName") String measurementName);
